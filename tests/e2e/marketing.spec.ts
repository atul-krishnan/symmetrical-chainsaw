...
test("home page messaging", async ({ page }) => {
  ...
  await expect(
    page.getByRole("heading", {
      name: /From policy text to role-ready training in under 45 minutes/i,
    }),
  ).toBeVisible();
  await page.getByRole("link", { name: /Review Pilot Scope/i }).first().click();
  await expect(page).toHaveURL(/\/pilot$/);
});

test("security page loads", async ({ page }) => {
  await page.goto("/security");
  await expect(page.getByRole("heading", { name: /Secure-by-default controls/i })).toBeVisible();
});
